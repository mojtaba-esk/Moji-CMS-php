...
*/
	...
	$SQL = 'SELECT * 
			...
			WHERE
				'. $srchSQL .'
			ORDER BY 
				'. $sortSQL .' `'. Module::$opt['admnOrdrBy'] .'` '. Module::$opt['admnOrdrType'];
	...
	$tpl -> assign_vars( array(
		...
		'L_USER_ID'		=> rwTitle( 'userId'),
		'L_INSERT_TIME'	=> rwTitle( 'insrtTime'),
		'L_URL'			=> rwTitle( 'url'),
		'L_ACTIVE_TIME'	=> rwTitle( 'actvTime'),
		...
		'SEARCH_DISPLAY'	=> true,//isset( $_GET['srch']) ? 'block' : 'none',
	...
	foreach( $rws as $key => $rw)
	{
		$tpl -> assign_block_vars( 'myblck',  array(
			...
			'USER_ID'		=> $rw[ 'userId'],
			'INSERT_TIME'	=> Lang::numFrm( Date::get( 'D d M Y G:i', $rw[ 'insrtTime'])),
			'URL'			=> $rw[ 'url'],
			'ACTIVE_TIME'	=> Lang::numFrm( Date::get( 'D d M Y G:i', $rw[ 'actvTime'])),
