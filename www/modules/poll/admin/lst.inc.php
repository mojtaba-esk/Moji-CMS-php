...
*/
	...
	$tpl -> assign_vars( array(
		...
		'L_QUESTION'	=> rwTitle( 'question'),
		'L_BRIEF'	=> rwTitle( 'body', 'brief'),
		'L_NEW'		=> Lang::getVal( 'new'),
		'L_PUBLISH_TIME' => rwTitle( 'pblishTime'),
		'L_START_TIME' => rwTitle( 'startTime'),
		'L_END_TIME' => rwTitle( 'endTime'),
		...
		'L_OPTIONS' => Lang::getVal( 'options'),
		'L_GRAPH' => Lang::getVal( 'graph'),		
	...
	if( is_array( $rws))
	{
		...
		foreach( $rws as $key => $rw)
		{
			$tpl -> assign_block_vars( 'myblck',  array(
				...
				'START_TIME' => Lang::numFrm( Date::get( 'D d M Y', $rw[ 'startTime'])),
				'END_TIME' => Lang::numFrm( Date::get( 'D d M Y', $rw[ 'endTime'])),
