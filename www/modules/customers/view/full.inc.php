...
*/
			...
			$SQL = 'SELECT `m`.*, `c`.`title` AS `catTitle`
				FROM
					...
					ON 
						...
						`c`.`lngId` = `m`.`lngId`
				WHERE 
					`m`.`rltdId` = '. intval( $_GET['id']) .' AND
					`m`.`lngId` = '. Lang::viewId().' AND 
					`m`.`pblishTime` > 0 AND 
					`m`.`pblishTime` <= '. time();
	...
	//<!-- Send vars to Template . . .
		...
		$tpl -> assign_vars( array(
			...
			'PAGE_TITLE'			=> Lang::getVal( 'siteTitle') .' | '. Lang::getVal( Module::$name) .' | '. $rw['title'],
			'PAGE_KEYWORDS'	=> Lang::getVal( 'siteTitle') .','.	Lang::getVal( Module::$name) .','.	$rw['title'],
			'PAGE_DESC'				=> Lang::getVal( 'siteTitle') .' - '. Lang::getVal( Module::$name) .' - '. briefStr( isset( $rw['lead']) ? $rw['lead'] : strip_tags( $rw['body']), 80),
