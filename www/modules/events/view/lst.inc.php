...
*/
	...
	//printr( $SQL);
	/*//insert time(in empty)
	$instSQL = '1';
	if( isset( $_GET['events']))
	{
		while( ( $cols = $inpt -> getRow()) && $cols['lngId'] != Lang::id());
	
		isset( $inst) or $inst = new Events( Module::$opt[ 'id']);
		empty( $cols['query']) or $instSQL = '`rltdId` IN ( '. $inst -> getIdsSQL( $cols['query'], Lang::viewId()) .')';
		isset( $cols['pblishTimeBgn']) and $cols['pblishTimeBgn'] = Date::mkTime( $cols['pblishTimeBgn']) and $instSQL .= ' AND `pblishTime` >= '. $cols['pblishTimeBgn'];
		if( isset( $cols['pblishTimeEnd']))
		{
			if( !empty( $cols['pblishTimeEnd']['G']))
			{
				empty( $cols['pblishTimeEnd']['G']) and $cols['pblishTimeEnd']['G']	= 23;
				empty( $cols['pblishTimeEnd']['i']) and $cols['pblishTimeEnd']['i']	= 59;
			$cols['pblishTimeEnd'] = Date::mkTime( $cols['pblishTimeEnd']) and $instSQL .= ' AND `pblishTime` <= '. $cols['pblishTimeEnd'] ;
			}
	...
	if( is_array( $rws))
	{
		foreach( $rws as $key => $rw)
		{
			$tpl -> assign_block_vars( 'myblck',  array(
				...
				'URL' => URL::rw( '?lng='. Lang::$info['shortName'] .'&md='. Module::$name .'&mod=full&id='. $rw['rltdId'] .'&t='. URL::clr( $rw['title'])),
