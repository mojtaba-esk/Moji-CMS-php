...
*/
	...
	//<!-- Insert new rows
		...
		if( $_GET['mod'] == 'new' && isset( $_POST['submit']))
		{
			...
			while( $cols = $inpt -> getRow())
			{
				...
				$cols[ 'ordrId'] 	= 65535;//The biggest number in two bytes.
	...
	$tpl -> set_filenames( array(
		'edit' => $_GET['sub'] .'.sub.'. Module::$name .'.admin.edit',
		)
	);
	...
	//<!-- Fetch the second Item title...
	
		$itmFRws = DB::load(
			array( 
				'tableName' => Module::$name .'_fields',
				'cols' => array( 'title'),
				'where' => array(
					'id' => intval( $_GET['fldId']),
				),
			)
		);
	...
	$tpl -> assign_vars( array(
		...
		'ITEM_TITLE'=> '<a href="?md='. Module::$name .'&sub=fields&typeId='. $_GET['typeId'] .'">'.$itmRws[0]['title'] .'</a> ('. $itmFRws[0]['title'] .')',
		
		'RETURN_URL'	=> '?md='. Module::$name .'&sub='. $_GET['sub'] .'&typeId='. $_GET['typeId'] .'&fldId='. $_GET['fldId'],
	 ...
	 */		
		...
		$form[] = $inpt -> hidden( 'fldId', 0, $_GET['fldId']);
