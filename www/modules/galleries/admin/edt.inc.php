	...
	//<!-- Insert new rows
		...
		if( $_GET['mod'] == 'new' && isset( $_POST['submit']))
		{
			...
			while( $cols = $inpt -> getRow())
			{
				...
				$cols[ 'pblishTime'] = $cols[ 'insrtTime'] = time();
				DB::insert( array(
			...
			$galleryLnk = '<br /><a href="./?md='. Module::$name .'&sub=gallery&itemId='. $rltdId .'">'. Lang::getVal( 'goGallery' ) .'</a><br />';
			msgDie( Lang::getVal( 'inserted') . $galleryLnk , './?md='. Module::$name, 30);
			return;
	...
	//<!-- Update rows
		...
		if( $_GET['mod'] == 'edt' && isset( $_POST['submit']))
		{
			...
			while( $cols = $inpt -> getRow())
			{
				...
				$cols[ 'forSale'] = intval( @$cols[ 'forSale']);//Das ist ein Check Box;
