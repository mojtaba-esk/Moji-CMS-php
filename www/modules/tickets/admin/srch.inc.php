...
*/
	...
	$tpl -> assign_vars( array(
			...
			'SEARCH_FORM_ELEMENTS' =>	$inpt -> text( 'query', Lang::id(), array( 'dir' => Lang::$info['dir'], 'align' => Lang::$info['align'], 'size' => 40)) . 
															...
															$inpt -> date( 'insrtTimeBgn', Lang::id(), array( 
																				'elmnts' => 'Y,M,d',
																				//'type' => 'jalali',
																				//'value' => 'now',
																				'difY'		=> array( -15, 1),
															...
															$inpt -> date( 'insrtTimeEnd', Lang::id(), array( 
																				'elmnts' => 'Y,M,d',
																				//'type' => 'jalali',
																				//'value' => 'now',
																				'difY'		=> array( -15, 1),
	...
	if( isset( $_GET['srch']))
	{
		...
		empty( $cols['query']) or $srchSQL = '`id` IN ( '. $srch -> getIdsSQL( $cols['query'], Lang::viewId()) .')';
		
		isset( $cols['insrtTimeBgn']) and $cols['insrtTimeBgn'] = Date::mkTime( $cols['insrtTimeBgn']) and $srchSQL .= ' AND `insrtTime` >= '. $cols['insrtTimeBgn'];
		if( isset( $cols['insrtTimeEnd']))
		{
			if( !empty( $cols['insrtTimeEnd']['Y']))
			{
				empty( $cols['insrtTimeEnd']['M']) and $cols['insrtTimeEnd']['M']	= 12;
				empty( $cols['insrtTimeEnd']['d']) and $cols['insrtTimeEnd']['d']	= 31;
			}

			$cols['insrtTimeEnd'] = Date::mkTime( $cols['insrtTimeEnd']) and $srchSQL .= ' AND `insrtTime` <= '. $cols['insrtTimeEnd'];

		}//End of if( isset( $cols['insrtTimeEnd']));
