	...
	//<!-- List of posts...
		...
		for( $i = 0; $i != $total; $i++)
		{
			$tpl -> assign_block_vars( 'postsblck',  array(
				...
				'RW' => Lang::numFrm( $total - $i),
				'RWID' => $i,
				'RW_ODD' => intval( $rws[ $i ]['userId'] != 0), // supporters have different color
				'ID' => & $rws[ $i ]['id'],

				'IP' 	 => & $rws[ $i ]['ip'],
				'WRITER' => $rws[ $i ]['userId'] ? ( '<a href="?md=users&sub=adminUsers&mod=view&id='. $rws[ $i ]['userId'] .'">'. $rws[ $i ]['writer'] .'</a>') : '',
				'BODY'	 => nl2br( $rws[ $i ]['body']),
				'PUBLISH_TIME' => Lang::numFrm( Date::get( 'D d M Y - G:i', $rws[ $i ][ 'insrtTime'])),

				'EXTRA_CLASS'	=> '',
				
				'ATTCHMNT'		=> $rws[ $i ]['atchId'] ? ( '[ <a href="?md='. Module::$name .'&mod=download&id='. $rws[ $i ][ 'atchId'] .'" title="'. $rws[ $i ]['fileName'] .'">'. Lang::getVal( 'attchmnt') .'</a> ]') : '',
