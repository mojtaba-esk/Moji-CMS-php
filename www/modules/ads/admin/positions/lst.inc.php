...
*/
	...
	/*SELECT * FROM `ads_positions` AS `pos`   INNER JOIN  `modules` AS `md` ON `pos`.`mdId`=`md`.`id`*/
	
	$SQL = 'SELECT  `pos`.`id`, `pos`.`mdId`, `pos`.`key`, `md`.`name`  FROM `'. Module::$name . '_'. $_GET['sub'] .'` AS `pos` INNER JOIN '.
		' `modules` AS `md` ON `pos`.`mdId`=`md`.`id`  
