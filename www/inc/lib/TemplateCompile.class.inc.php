 ...
 */
		//<!-- Load the template From DB
			...
			$tplFileRw = DB::load( 
				array( 
					'tableName' => 'templates_contents',
					'where' => array(
						'name'	=>	& $this -> template -> filename[ $handle ],
						'tmpId'	=>	& $this -> template -> tmpId,
					),
				));

		//End of Load the template From DB -->
		
		if( !$tplFileRw && defined( 'DEBUG_MODE'))
		{
			printr( 'The template[ '. $this -> template -> filename[ $handle ] .' ] with ID [ '. $this -> template -> tmpId .' ], Not found in DataBase!');
			return NULL;
		}

		// $this->template->compiled_code[$handle] = $this->compile(trim(@file_get_contents($this->template->files[$handle])));
		$this -> template -> compiled_code[ $handle ] = $this -> compile( trim( $tplFileRw[0]['content']));
...
*//*
	...
	*/
	function compile_var_tags(&$text_blocks)
	{
		...
		$text_blocks = preg_replace('#\{([a-z0-9\-_]*)\}#is', "<?php echo @\$this->_rootref['\\1']; ?>", $text_blocks);
