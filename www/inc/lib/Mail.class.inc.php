...
@comment	Modified by Mojtaba Eskandari on 2009-09-02;
*/
...
function from( $from )
{
	...
	if( defined( 'DEBUG_MODE') && !is_string( $from))
	{
		printr( 'Class Mail: error, From is not a string');
		exit;
	}
 ...
 */
function body( $body, $charset='utf-8' )
{
	$this->body = $body;
...
function isValidEmail($address)
{
	...
	//Modified By Moji...
	return preg_match('/^[\w\.\-]+@([\w\-]+\.)+[a-zA-Z]{2,4}$/', strtolower( $address), $matches);
...
function checkAdresses( $aad )
{
	for($i=0;$i< count( $aad); $i++ ) {
		if( defined( 'DEBUG_MODE') && ! $this->isValidEmail( $aad[$i]))
		{
			printr( "Class Mail, method Mail : invalid address $aad[$i]");
			exit;
		}
	}
}
...
function _build_attachement()
{
	...
	for( $i=0; $i < count( $this->aattach); $i++ ) {
		...
		if(  defined( 'DEBUG_MODE') && !file_exists( $filename))
		{
			printr( "Class Mail, method attach : file $filename can't be found");
			exit;
		}
