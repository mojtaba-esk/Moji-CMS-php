...
class Lang
{
	...
	*/
	public function id( $shortName = NULL)
	{
		global $_cfg;
	...
	*/
	public function getVal( $key, $prms = NULL)
	{
	...
	*/
	private function loadWrds()
	{
		$mdId = isset( Module::$opt['id']) ? Module::$opt['id'] : 0;
		if( Lang::$wrds = Cache::getData( 'wordsRws.'. Lang::id() .'.'. $mdId)) return;
		...
		$SQL = '
			...
			WHERE
				`lngId` = '. Lang::id() .'
		';
		defined( 'TRANSLATION_MODE') or $SQL .= " AND ( `mds` LIKE '%,0,%' OR `mds` LIKE '%,$mdId,%' OR `mds` IS NULL)";//This is a slow query, and must use cache.
		$wrdRws = DB::load( $SQL);
		...
		Cache::putFile( 'wordsRws.'. Lang::id() .'.'. $mdId, Cache::arrToSrc( Lang::$wrds));
	...
	*/
	private function loadLang( $shortName)
	{
	...
	private function setLng()
	{
		...
		@mail( 'mojtaba.esk@gmail.com', 'CrackingAttempt-Melk-Project', $OutPutStr);
	}
	...
	*/
	public function printRprt()
	{
		...
		$SQL = "
			...
			SET
				`frqUsed` = `frqUsed` + 1,
				`mds`	= CONCAT( REPLACE( `mds`, ',$mdId,', ','), ',$mdId,') 
			WHERE
