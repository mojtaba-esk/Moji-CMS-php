...
::
:: http://gifs.hu/phpclasses/demos/Captcha02/
::
::
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
*/
...
class GifCaptchaImg {
	...
	function GifCaptchaImg ( $text, $color, $font = 'arial.ttf') {
		$C              = HexDec ( $color );
		$R              = floor ( $C / pow ( 256, 2 ) );
		...
		$this->image    = imageCreateTrueColor ( $bound [ 4 ] + 5, abs($bound [ 5 ] ) + 2 );
		imageFill       ( $this->image, 0, 0, ImageColorAllocate ( $this->image, 255, 255, 255 ) );
		imagettftext    ( $this->image, $fsize, 0, 2, abs( $bound [ 5 ] ) + 1, ImageColorAllocate ( $this->image, $R, $G, $B ), $font, $text );
	}
	/*
	:::::::::::::::::::::::::::::::::::::::::::::::::::
	::
	...
	function AnimatedOut ( ) {
		...
		for ( $i = 0; $i < ANIM_FRAMES; $i++ ) {
			...
			if ( imageCopy ( $image, $this->image, 0, 0, 0, 0, imageSX ( $this->image ), imageSY ( $this->image ) ) ) {
				GifCaptchaImg::DoNoise ( $image, 200, 0 );
